...
This report identifies the specific incremental changes that need to be deployed to update the existing Supabase project.
...
- **New Tools**: `commerce.confirmConsent`, `commerce.resumeCart`, `loopkitchen_share`.
- **Updated Tools**: `commerce.confirmOrder` (idempotency), `loopkitchen_recipes` (share meta).
...
**Action:** Redeploy `commerce_router` (often part of `mcp-server` deployment if shared code changed).
