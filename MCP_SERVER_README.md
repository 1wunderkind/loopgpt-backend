# LooptOS MCP Server Documentation

## Overview

This is the Model Context Protocol (MCP) server for LooptOS, the Agentic Commerce intelligence layer. It is designed for integration with the ChatGPT App Store. The MCP server acts as a service layer that exposes 19 specialized meal planning, weight tracking, delivery, and geolocation tools to ChatGPT.
...
The MCP server is currently configured with
`"authentication": { "type": "none" }` for development and testing.
...
**Fix:** Ensure `supabase/manifest.json` exists and is committed to the
repository.
...
**Fix:** The MCP server already includes CORS headers. Ensure you're using the
correct URL.
...
_Built with ❤️ for the ChatGPT App Store_
