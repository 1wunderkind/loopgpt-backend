...
This document details the implementation of Step 6, which introduces a robust
state management layer for commerce flows.

## 1. Architecture Overview

The system now persists all commerce interactions in a `commerce.cart_sessions`
table. This enables:
...
3. **Confirm Order** -> Checks session status, executes order, updates to
   `confirmed`.
4. **Failover** -> If primary fails, checks `allow_failover` flag and retries
   with alternatives.
...
- **Behavior**: Now persists the routing result as a `cart_session` with status
  `awaiting_consent`.
...
- **Behavior**: Validates session and updates status to
