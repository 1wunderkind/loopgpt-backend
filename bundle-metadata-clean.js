...
const convenienceFunctionsStart = indexLines.findIndex((line) =>
  line.includes("// CONVENIENCE FUNCTIONS")
);
const indexConvenienceFunctions = indexLines.slice(convenienceFunctionsStart)
  .join("\n");
...
const hasGetCompleteMetadata = bundled.includes(
  "export function getCompleteMetadata",
);
const hasGetToolWithRouting = bundled.includes(
  "export function getToolWithRouting",
);
...
const hasOrphanedBrackets = bundled.match(/^\s*\]\s*$/m) ||
  bundled.match(/^\s*\};\s*$/m);
console.log("✅ No orphaned brackets:", !hasOrphanedBrackets ? "YES" : "NO");
