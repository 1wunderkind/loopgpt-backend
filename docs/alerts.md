...
2. **Provider Outage:** If `PROVIDER` errors are high, check status pages
   (MealMe, OpenAI).
   - **Action:** Enable `DISABLE_ORDERING` kill switch if orders are failing
     consistently.
...
2. **Validation:** If a specific client version is sending bad data, contact
   client team.
