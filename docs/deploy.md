...
We use GitHub Actions for Continuous Integration. Every PR and push to `main` triggers:
...
Deploy from your feature branch or `main` to the staging environment to verify changes.
...
**Critical:** Only deploy to production from the `main` branch after verifying on staging.
...
If a deployment introduces a critical regression, follow these steps immediately:
