...
- **Service Role:** Has full access to all tables (bypasses RLS). Used by Edge
  Functions and administrative tools.
- **Authenticated Users:** Can access their own data via `auth.uid()`.
- **Anonymous Users:** Generally have NO access, except for specific public
  read-only tables.
...
_Access: strictly scoped to `user_id = auth.uid()`_
...
_Access: Public read-only, or restricted_
...
1. **Migration Reviews:** All new tables must include RLS enablement and
   policies in the migration file.
2. **CI Checks:** (Planned) Automated check to ensure `row_security` is enabled
   on all tables.
