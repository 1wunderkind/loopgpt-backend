...
- This simplifies the model for B2C use cases while allowing future expansion to
  Workspaces (B2B) if needed.

## Isolation Strategy

1. **Logical Isolation:** All data is stored in shared tables with a `tenant_id`
   (or `user_id`) column.
2. **Row Level Security (RLS):** Database policies enforce that a tenant can
   only access their own rows.
3. **Runtime Enforcement:** All Edge Functions require a resolved `tenantId`
