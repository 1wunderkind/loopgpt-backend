/**
 * LooptOS Metadata Validation Tests
 *
...
Deno.test("App metadata - has required fields", () => {
  assertExists(LOOPTOS_METADATA.name);
  assertExists(LOOPTOS_METADATA.tagline);
  assertExists(LOOPTOS_METADATA.shortDescription);
  assertExists(LOOPTOS_METADATA.longDescription);
  assertExists(LOOPTOS_METADATA.keywords);
  assertExists(LOOPTOS_METADATA.categories);

  assert(
    LOOPTOS_METADATA.keywords.length > 0,
    "Keywords should not be empty",
  );
  assert(
    LOOPTOS_METADATA.categories.length > 0,
...
Deno.test("App metadata - description lengths are appropriate", () => {
  assert(
    LOOPTOS_METADATA.shortDescription.length <= 200,
    "Short description should be <= 200 chars",
  );
  assert(
    LOOPTOS_METADATA.longDescription.length >= 100,
    "Long description should be >= 100 chars",
  );
  assert(
    LOOPTOS_METADATA.tagline.length <= 80,
    "Tagline should be <= 80 chars",
  );
});

Deno.test("App metadata - keywords are valid", () => {
  for (const keyword of LOOPTOS_METADATA.keywords) {
...
Deno.test("Tool descriptions - all tools have required fields", () => {
  for (const [toolId, tool] of Object.entries(ALL_TOOL_DESCRIPTIONS)) {
    ...
    assertExists(
      tool.primaryDescription,
      `Tool ${toolId} missing primaryDescription`,
    );
    assertExists(tool.whenToUse, `Tool ${toolId} missing whenToUse`);
    assertExists(tool.whenNotToUse, `Tool ${toolId} missing whenNotToUse`);
    assertExists(
      tool.uniqueCapabilities,
      `Tool ${toolId} missing uniqueCapabilities`,
    );
    ...
    assert(
      ...
      `Tool ${toolId} whenToUse should not be empty`,
    );
    assert(
      tool.whenNotToUse.length > 0,
      `Tool ${toolId} whenNotToUse should not be empty`,
    );
    assert(
      tool.uniqueCapabilities.length > 0,
      `Tool ${toolId} uniqueCapabilities should not be empty`,
    );
  }
});
...
Deno.test("Tool descriptions - parameters have required fields", () => {
  for (const [toolId, tool] of Object.entries(ALL_TOOL_DESCRIPTIONS)) {
    for (const param of tool.requiredParams) {
      ...
      assertExists(
        param.description,
        `Tool ${toolId} required param missing description`,
      );
      assertExists(
        param.example,
        `Tool ${toolId} required param missing example`,
      );
    }
    ...
    for (const param of tool.optionalParams) {
      ...
      assertExists(
        param.description,
        `Tool ${toolId} optional param missing description`,
      );
    }
  }
});
...
Deno.test("Tool descriptions - return formats are documented", () => {
  for (const [toolId, tool] of Object.entries(ALL_TOOL_DESCRIPTIONS)) {
    assertExists(
      tool.returnFormat.description,
      `Tool ${toolId} returnFormat missing description`,
    );
    assertExists(
      tool.returnFormat.fields,
      `Tool ${toolId} returnFormat missing fields`,
    );
    assert(
...
Deno.test("Routing hints - has trigger hints", () => {
  ...
  assert(
    hintCount >= 15,
    `Expected at least 15 trigger hints, got ${hintCount}`,
  );
});
...
Deno.test("Routing hints - all hints have required fields", () => {
  for (const [hintKey, hint] of Object.entries(ROUTING_METADATA.triggerHints)) {
    ...
    assert(
      hint.examples.length >= 5,
      `Hint ${hintKey} should have at least 5 examples`,
    );
    assert(
      hint.confidence >= 0 && hint.confidence <= 1,
      `Hint ${hintKey} confidence should be 0-1`,
    );
    assert(
      hint.relatedTools.length > 0,
      `Hint ${hintKey} should have at least 1 related tool`,
    );
  }
});
...
Deno.test("Routing hints - examples are diverse", () => {
  for (const [hintKey, hint] of Object.entries(ROUTING_METADATA.triggerHints)) {
    ...
    const uniqueStarts = new Set(
      examples.map((ex) => ex.slice(0, 10).toLowerCase()),
    );
...
Deno.test("Routing hints - negative hints are documented", () => {
  ...
  assert(
    ROUTING_METADATA.negativeHints.length >= 5,
    "Should have at least 5 negative hints",
  );
  ...
  for (const negHint of ROUTING_METADATA.negativeHints) {
    ...
    assert(
      negHint.examples.length >= 3,
      "Negative hint should have at least 3 examples",
    );
  }
});
...
Deno.test("Routing hints - tool chains are valid", () => {
  ...
  for (const chain of ROUTING_METADATA.toolChains) {
    ...
    assert(
      chain.sequence.length >= 2,
      `Tool chain ${chain.name} should have at least 2 tools`,
    );
...
Deno.test("Tool summary - category counts are accurate", () => {
  ...
  const totalFromCategories = Object.values(categories).reduce(
...
Deno.test("Performance - getToolDescription is fast", () => {
  ...
  assert(
    duration < 100,
    `1000 lookups should take < 100ms, took ${duration}ms`,
  );
});
...
Deno.test("Performance - searchTools is reasonable", () => {
  ...
  assert(
    duration < 500,
    `100 searches should take < 500ms, took ${duration}ms`,
  );
});
...
console.log("\n=".repeat(80));
console.log("LooptOS Metadata Validation Summary");
console.log("=".repeat(80));
console.log(`✅ Total Tools: ${getToolCount()}`);
console.log(
  `✅ Trigger Hints: ${Object.keys(ROUTING_METADATA.triggerHints).length}`,
);
