...
function getFoodFlags(group: string, name: string): FoodEntry["flags"] {
  ...
  if (group === "fat") {
    ...
    const isAnimalFat = animalFats.some((af) =>
  ...
  if (group === "condiment") {
    const animalCondiments = [
      "mayo",
      "mayonnaise",
      "fish sauce",
      "oyster sauce",
    ];
    const isAnimalCondiment = animalCondiments.some((ac) =>
...
export const FOOD_DATABASE: Record<string, FoodEntry> = {\n`;
  ...
  for (const [canonicalName, entry] of sortedEntries) {
    ...
    code += `    },\n`;
    code += `    flags: ${
      JSON.stringify(entry.flags, null, 6).replace(/\n/g, "\n    ")
    },\n`;
    code += `  },\n`;
  }
...
async function main() {
  ...
  // Write output
  const outputPath =
    "./supabase/functions/_shared/nutrition/dictionary.generated.ts";
