...
const BASE_URL = Deno.env.get("STAGING_URL") || "http://localhost:54321/functions/v1";
...
async function runTest(operation: string, endpoint: string, payload: any): Promise<TestResult> {
  ...
  try {
    ...
    // SIMULATION MODE
    await new Promise(resolve => setTimeout(resolve, 100 + Math.random() * 500)); // 100-600ms latency
...
async function runSoak() {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    results.push(await runTest("nutrition", "/mcp-tools/nutrition", { recipes: [{ name: "Test" }] }));
    
    // 2. Meal Plan
    results.push(await runTest("mealplan", "/mcp-tools/mealplan", { goals: { calories: 2000 } }));
    
    // 3. Get Quotes (Mock)
    results.push(await runTest("quotes", "/mealme_get_quotes", { cartId: "test-cart", mode: "groceries" }));
  }
