...
export async function loadUSDAData(dataDir: string): Promise<USDAFood[]> {
  ...
  const foodCsvPath =
    `${dataDir}/FoodData_Central_sr_legacy_food_csv_2018-04/food.csv`;
  const foodCsv = await Deno.readTextFile(foodCsvPath);
  const foodRows = await parse(foodCsv, {
    skipFirstRow: true,
    columns: [
      "fdc_id",
      "data_type",
  ...
  const categoryCsvPath =
    `${dataDir}/FoodData_Central_sr_legacy_food_csv_2018-04/food_category.csv`;
  const categoryCsv = await Deno.readTextFile(categoryCsvPath);
  const categoryRows = await parse(categoryCsv, {
    skipFirstRow: true,
  ...
  // Load nutrients
  const nutrientCsvPath =
    `${dataDir}/FoodData_Central_sr_legacy_food_csv_2018-04/food_nutrient.csv`;
  const nutrientCsv = await Deno.readTextFile(nutrientCsvPath);
  const nutrientRows = await parse(nutrientCsv, {
    skipFirstRow: true,
    columns: [
      "id",
      "fdc_id",
  ...
  console.log(
    `✅ Loaded ${foodRows.length} foods, ${nutrientRows.length} nutrient values`,
  );
 ...
 */
export function filterCommonFoods(
  foods: USDAFood[],
  limit: number,
): USDAFood[] {
  ...
  const filtered = foods.filter((food) => {
    ...
    if (
      food.nutrients.kcal === 0 && food.nutrients.protein === 0 &&
  ...
  const sampleCats = foods.slice(0, 10).map((f) =>
    `${f.categoryId}:${f.category}`
  );
  console.log(`📊 Sample category IDs: ${sampleCats.join(", ")}`);
  console.log(
    `📊 Filtered to ${filtered.length} basic foods from ${foods.length} total`,
  );
