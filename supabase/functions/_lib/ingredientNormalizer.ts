...
function extractQuantityAndUnit(
  ingredient: string,
): { qty: number; unit?: string; remainder: string } {
  ...
  if (match) {
    ...
    return {
      qty,
      unit: undefined,
      remainder: match[2] ? `${match[2]} ${remainder}` : remainder,
    };
  }
 ...
 */
export function toMealMeCartItems(
  ingredients: NormalizedIngredient[],
): Array<{ name: string; qty: number; skuHint?: string }> {
