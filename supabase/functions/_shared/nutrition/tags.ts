 ...
 */
function hasIngredientFlag(
  canonicalNames: string[],
  flagName: keyof IngredientFlags,
): boolean {
...
export function getMacroBasedTags(perServing: Macros): DietTag[] {
  ...
  if (
    perServing.fiber_g !== undefined &&
    perServing.fiber_g >= DIET_TAG_THRESHOLDS.HIGH_FIBER_MIN_G
  ) {
  ...
  if (
    perServing.sodium_mg !== undefined &&
    perServing.sodium_mg < DIET_TAG_THRESHOLDS.LOW_SODIUM_MAX_MG
  ) {
  ...
  if (
    perServing.sugar_g !== undefined &&
    perServing.sugar_g < DIET_TAG_THRESHOLDS.LOW_SUGAR_MAX_G
  ) {
    tags.push("low_sugar");
  }
 ...
 */
export function checkDietViolations(
  dietTags: DietTag[],
  forbiddenTags: DietTag[],
): boolean {
...
export const DIET_TAG_DESCRIPTIONS: Record<DietTag, string> = {
  ...
  // Macro-based
  "low_carb":
    `Low in carbohydrates (less than ${DIET_TAG_THRESHOLDS.LOW_CARB_MAX_G}g per serving)`,
  "high_protein":
    `High in protein (at least ${DIET_TAG_THRESHOLDS.HIGH_PROTEIN_MIN_G}g per serving)`,
  "high_fiber":
    `High in fiber (at least ${DIET_TAG_THRESHOLDS.HIGH_FIBER_MIN_G}g per serving)`,
  "keto_friendly": `Keto-friendly (very low carb, high fat)`,
  "low_fat":
    `Low in fat (less than ${DIET_TAG_THRESHOLDS.LOW_FAT_MAX_G}g per serving)`,
  "low_sodium":
    `Low in sodium (less than ${DIET_TAG_THRESHOLDS.LOW_SODIUM_MAX_MG}mg per serving)`,
  "low_sugar":
    `Low in sugar (less than ${DIET_TAG_THRESHOLDS.LOW_SUGAR_MAX_G}g per serving)`,
  ...
  "mediterranean":
    "Mediterranean-style (rich in olive oil, fish, and vegetables)",
};
 ...
 */
export function getDietTagDescriptions(
  tags: DietTag[],
