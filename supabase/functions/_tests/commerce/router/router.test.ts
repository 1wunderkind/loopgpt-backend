  ...
  await withTestEnv(
    ...
    async () => {
      ...
      assertEquals(
        providers.length >= 4,
        true,
        "Should have at least 4 providers enabled",
      );
    },
  );
});
...
Deno.test("Scorer - Selects cheapest provider with price optimization", async () => {
  await withTestEnv(
    ...
    async () => {
      ...
      const scored = await scorer.scoreProviders(
        quotes,
        SAMPLE_ITEMS.length,
        "price",
      );
  ...
  await withTestEnv(
    ...
    async () => {
      ...
      assertEquals(
        scored[0].provider.id,
        "MEALME",
        "Should select provider with higher commission",
      );
    },
  );
});
  ...
  await withTestEnv(
    ...
    async () => {
      ...
      assertEquals(
        scored[0].provider.id,
        "KROGER_API",
        "Should select provider with priority boost",
      );
    },
  );
});
