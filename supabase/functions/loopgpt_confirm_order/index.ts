...
async function attemptConfirmation(
  ...
  try {
    console.log(
      `[ConfirmOrder] Attempting confirmation with provider: ${providerId}`,
    );
    ...
    if (shouldFail) {
      throw new Error(
        "PROVIDER_UNAVAILABLE: Simulated provider failure for testing",
      );
    }

    const orderId = `order_${providerId.toLowerCase()}_${Date.now()}`;
    const trackingUrl =
      `https://track.${providerId.toLowerCase()}.com/${orderId}`;
    const estimatedDelivery = new Date(Date.now() + 45 * 60 * 1000)
      .toISOString();

    console.log(
      `[ConfirmOrder] ✅ Confirmation successful with ${providerId}: ${orderId}`,
    );
    ...
    console.error(
      `[ConfirmOrder] ❌ Confirmation failed with ${providerId}:`,
      error,
    );
    return {
      success: false,
...
async function recordOrderOutcome(
  ...
  try {
    ...
    const recordOutcomeUrl =
      `${supabaseUrl}/functions/v1/loopgpt_record_outcome`;
    ...
    if (!response.ok) {
      console.error(
        `[ConfirmOrder] Failed to record outcome: ${response.statusText}`,
      );
    } else {
      console.log(
        `[ConfirmOrder] Recorded outcome for ${providerId}: ${
...
const handler = async (req: Request) => {
  ...
  try {
    ...
    const primaryProviderId = request.routingContext?.primaryProviderId ||
      "INSTACART";
    const alternatives = request.routingContext?.alternatives || [];

    console.log(`[ConfirmOrder] Primary provider: ${primaryProviderId}`);
    console.log(
      `[ConfirmOrder] Alternatives: ${
        alternatives.map((a) => a.providerId).join(", ")
      }`,
    );
    ...
    if (!confirmResult.success && alternatives.length > 0) {
      ...
      if (shouldFailover) {
        console.log(
          `[ConfirmOrder] Primary provider failed with retryable error, attempting failover...`,
        );
        ...
        console.log(
          `[ConfirmOrder] Failing over from ${failoverFrom} to ${usedProviderId}`,
        );
        ...
        console.log(
          `[ConfirmOrder] Primary provider failed with non-retryable error, no failover attempted`,
        );
      }
    }
    ...
    if (confirmResult.success) {
      ...
      const response: ConfirmOrderResponse = {
        ...
        trackingUrls: confirmResult.trackingUrl
          ? [confirmResult.trackingUrl]
          : undefined,
      ...
      const errorMessage = failoverAttempted
        ...
        : `Unable to place order with ${primaryProviderId}. ${
          confirmResult.error?.message ||
          "Please check your payment method and delivery address."
        }`;
