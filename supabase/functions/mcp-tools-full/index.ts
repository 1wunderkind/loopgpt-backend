...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "POST" && toolMatch) {
      ...
      const userId = params.userId || req.headers.get("X-User-ID") ||
        "anonymous";

      // Rate limiting (skip for utility tools)
      if (
        ENV.ENABLE_RATE_LIMITING && !toolName.startsWith("health") &&
        !toolName.startsWith("usage")
      ) {
        try {
