...
export async function generateRecipes(params: unknown): Promise<RecipeList> {
  ...
  try {
    ...
    const systemPrompt =
      `You are TheLoopGPT's recipe generation engine. Generate creative, practical recipes based on the provided ingredients.
  ...
  try {
    ...
    if (ENV.ENABLE_CACHING) {
      ...
      const cached = await cacheGet<
        { recipes: RecipeList; nutrition: NutritionAnalysis }
      >(cacheKey);
      if (cached) {
        const duration = Date.now() - startTime;
        logToolSuccess(toolName, duration, { cached: true });
