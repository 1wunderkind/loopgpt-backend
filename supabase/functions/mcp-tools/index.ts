...
import {
  generateMealPlan,
  generateMealPlanWithGroceryList,
} from "./mealplan.ts";
...
import { cancelOrder, confirmOrder, prepareCart } from "./commerce.ts";
...
const MANIFEST = {
  ...
  description:
    "Complete food commerce platform: recipes, meal planning, grocery lists, and intelligent order routing",
  status:
    "Optimized: Postgres caching + streaming + smart routing + intent classification + commerce integration",
  tools: [
    {
      name: "health.check",
    ...
    },
    {
      name: "food.router",
      description:
        "🌟 PRIMARY ENTRYPOINT: Smart router for any food-related query. Automatically classifies intent (recipes/nutrition/meal planning/grocery) and routes to the appropriate tool. Use this for vague or natural-language queries like 'I'm hungry', 'what should I eat?', 'help me plan meals', etc. For specific requests, you can still use specialized tools directly.",
      status: "available",
      primary: true,
    },
    {
      ...
      description:
        "Generate creative recipes from available ingredients (direct access)",
      status: "available",
    },
    {
    ...
    },
    {
      ...
      description:
        "🛒 Prepare shopping cart and route order through intelligent provider selection. Detects missing ingredients (if pantry provided) and calls the LoopGPT Commerce Router to get best provider quotes with multi-factor scoring (price, speed, availability, margin, reliability).",
      status: "available",
    },
    {
      name: "commerce.confirmOrder",
      description:
        "Confirm and place order with selected provider using confirmation token from prepareCart",
      status: "available",
    },
    {
    ...
    },
    {
      ...
      description:
        "👤 Update user dietary preferences, calorie targets, and cuisine preferences. Stores preferences for personalized meal suggestions and retention features.",
      status: "available",
    },
    {
      name: "retention.dailySuggestion",
      description:
        "☀️ Generate personalized daily meal suggestions (1-3 recipes) based on user profile. Perfect for re-engagement and 'What should I eat today?' queries. Returns card-friendly format with CTAs.",
      status: "available",
    },
    {
      name: "retention.weeklyRefresh",
      description:
        "📅 Generate personalized weekly meal plan based on user profile. Updates lastPlanDate for retention tracking. Perfect for 'Refresh my weekly plan' or 'Plan my meals for the week' queries.",
      status: "available",
    },
    {
      name: "feedback.sentiment",
      description:
        "💬 Record user feedback on recipes, meal plans, and grocery lists. Supports: 👍 Helpful / 👎 Not Helpful, ⭐ Star Ratings (1-5), ❤️ Favorites. Use this to capture user sentiment for analytics and personalization.",
      status: "available",
    },
    {
      name: "feedback.getFavorites",
      description:
        "❤️ Retrieve user's favorited content (recipes, meal plans, grocery lists). Returns list of favorites with metadata.",
      status: "available",
    },
    {
      name: "feedback.getStats",
      description:
        "📊 Get aggregated sentiment statistics for content. Returns helpful percentage, average rating, favorite count, etc. Useful for ranking and analytics.",
      status: "available",
    },
    {
      name: "loopkitchen.recipes.generate",
      description:
        "🍳 LoopKitchen: Generate creative recipes with chaos mode (1-10 playfulness rating), soft time/diet constraints, and widget-based UI. Returns RecipeCardCompact widgets with slug IDs, vibe normalization, and constraint flags.",
      status: "available",
    },
    {
      name: "loopkitchen.recipes.details",
      description:
        "📖 LoopKitchen: Get detailed recipe with instructions, ingredient split (have vs need), parallel nutrition analysis, and grocery list. Returns RecipeCardDetailed + NutritionSummary + GroceryList widgets.",
      status: "available",
    },
    {
      name: "loopkitchen.nutrition.analyze",
      description:
        "🥗 LoopKitchen: Standalone nutrition analysis from recipes or raw ingredients. Returns NutritionSummary widget with health score, confidence indicators, insights, and warnings. Supports both recipe-based and ingredient-based analysis.",
      status: "available",
    },
    {
      name: "loopkitchen.nutrition.logMeal",
      description:
        "📝 LoopKitchen: Log meal with nutrition data for daily/weekly tracking (planned for Phase 4 database integration).",
      status: "planned",
    },
    {
      name: "loopkitchen.nutrition.daily",
      description:
        "📊 LoopKitchen: Get daily nutrition summary aggregating all logged meals (planned for Phase 4 database integration).",
      status: "planned",
    },
    {
      name: "loopkitchen.mealplan.generate",
      description:
        "🗓️ LoopKitchen: Generate 7-day meal plan using MealPlannerGPT. Returns WeekPlanner widget with breakfast/lunch/dinner for each day, calorie targets, and weekly summary. Optimizes for ingredient reuse and diet preferences.",
      status: "available",
    },
    {
      name: "loopkitchen.mealplan.withGrocery",
      description:
        "🛍️ LoopKitchen: Generate meal plan with aggregated grocery list. Returns WeekPlanner + GroceryList widgets. Uses GroceryGPT to organize ingredients into categories.",
      status: "available",
    },
    {
      name: "loopkitchen.mealplan.prepareOrder",
      description:
        "📦 LoopKitchen: Prepare grocery order from meal plan. Generates grocery list and gets provider quotes via commerce layer. Requires userId and location.",
      status: "available",
    },
    {
      name: "loopkitchen.mealplan.complete",
      description:
        "✨ LoopKitchen: Complete meal planning flow. Generates meal plan + grocery list + provider quotes in one call. Returns WeekPlanner, GroceryList, and commerce data. Ultimate convenience function.",
      status: "available",
    },
  ],
};
  ...
  try {
    ...
    if (req.method === "POST" && toolMatch) {
      ...
      try {
        ...
        const sessionId = params.sessionId || req.headers.get("x-session-id") ||
          `session-${Date.now()}`;
        ...
        logSessionEvent({
          ...
          console.error("[Analytics] Failed to log session event:", err)
        );
...
console.log(
  ...
  }] TheLoopGPT MCP Tools server started (minimal version)`,
);
