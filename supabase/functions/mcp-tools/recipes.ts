...
import { addCtasToResponse, generateRecipesCtas } from "./ctaSchemas.ts";
import {
 ...
 */
export async function generateRecipesWithNutrition(
  ...
  try {
    console.log(
      "[recipes.generateWithNutrition] Starting composite operation...",
    );
    ...
    const recipesWithNutrition = recipes.map(
      (recipe: { id: string; [key: string]: unknown }) => {
        const analysis = analyses.find((a: { recipeId: string }) =>
    ...
    console.log("[recipes.generateWithNutrition] Success", {
    ...
    const duration = Date.now() - startTime;
    console.error("[recipes.generateWithNutrition] Error", {
...
export async function generateRecipes(params: Record<string, any>) {
  ...
  try {
    ...
    if (isLowEffort) {
      ...
      systemPrompt =
        `Generate ${input.maxRecipes} QUICK and EASY recipe(s) using common pantry items. Focus on:
...
${
          input.dietaryTags.length > 0
            ? `Diet: ${input.dietaryTags.join(", ")}. `
            : ""
        }Tag all recipes with "low_effort" and "quick". Return JSON with recipes array.`;

      userPrompt =
        `Generate ${input.maxRecipes} quick and easy recipe(s) for someone who is tired or wants minimal effort.

Suggested ingredients (use what makes sense): ${
          input.ingredients.map((i: string | { name: string }) =>
...
Examples: scrambled eggs, pasta with butter and cheese, rice bowl, quesadilla, instant ramen upgrade`;
      ...
      // Normal mode
      systemPrompt = `Generate ${input.maxRecipes} recipe(s) using: ${
        input.ingredients.map((i: string | { name: string }) =>
          typeof i === "string" ? i : i.name
        ).join(", ")
      }. ${
        ...
        input.excludeIngredients.length > 0
          ...
          : ""
      }Return JSON with recipes array.`;

      userPrompt =
        `Generate ${input.maxRecipes} recipe(s) using these ingredients:
${
          input.ingredients.map((
            i: string | { name: string; quantity?: string },
          ) =>
            `- ${typeof i === "string" ? i : i.name}${
              (typeof i !== "string" && i.quantity) ? ` (${i.quantity})` : ""
            }`
          ).join("\n")
        }
...
${input.difficulty !== "any" ? `Difficulty level: ${input.difficulty}` : ""}`;
    ...
    if (params.userId && recipes.length > 0) {
      console.log(
        "[recipes.generate] Scoring recipes with recommendation engine...",
      );

      // Prepare candidate recipes for scoring
      const candidateRecipes: CandidateRecipe[] = recipes.map((
        recipe: { id: string; name: string; ingredients: { name: string }[] },
      ) => ({
        recipe_id: recipe.id,
        title: recipe.name,
      ...
      recipes = recipes.map(
        ...
        (
          a: { recommendationScore?: number },
          b: { recommendationScore?: number },
        ) => {
