...
const NUTRITION_MAP: Record<string, any> = {
  ...
  "spinach": {
    ...
    "sugar_g": 0.1,
    "sodium_mg": 24,
    "micros": { "vitamin_a_mcg": 2813, "iron_mg": 0.8 },
  },
  "peanut butter": {
    "unit": "tbsp",
    "calories": 95,
  ...
  "almond": {
    ...
    "sugar_g": 0.04,
    "sodium_mg": 0.01,
    "micros": { "vitamin_e_mg": 0.26, "calcium_mg": 26.9 },
  },
  "pepper": {
    "unit": "piece",
    "calories": 24,
    ...
    "sugar_g": 3,
    "sodium_mg": 2,
    "micros": { "vitamin_c_mg": 127, "vitamin_a_mcg": 186 },
  },
  "cheese": {
    "unit": "g",
    "gramsPerUnit": 1,
...
console.log(
  `[TheLoop Nutrition] Loaded ${Object.keys(NUTRITION_MAP).length} ingredients`,
);
console.log(
  `[TheLoop Nutrition] Loaded ${
    Object.keys(INGREDIENT_TRANSLATIONS).length
  } translations`,
);
...
function computeNutrientsForIngredient(
  ...
  const nutrients: Record<string, number> = {
    calories: (ingredientData.calories || 0) *
      (ingredientData.gramsPerUnit ? grams : 1),
    protein_g: (ingredientData.protein_g || 0) *
      (ingredientData.gramsPerUnit ? grams : 1),
    carbs_g: (ingredientData.carbs_g || 0) *
      (ingredientData.gramsPerUnit ? grams : 1),
    fat_g: (ingredientData.fat_g || 0) *
      (ingredientData.gramsPerUnit ? grams : 1),
    fiber_g: (ingredientData.fiber_g || 0) *
      (ingredientData.gramsPerUnit ? grams : 1),
    sugar_g: (ingredientData.sugar_g || 0) *
      (ingredientData.gramsPerUnit ? grams : 1),
    sodium_mg: (ingredientData.sodium_mg || 0) *
      (ingredientData.gramsPerUnit ? grams : 1),
  };
  ...
  if (ingredientData.micros) {
    ...
    if (micros.vitamin_a_mcg) {
      nutrients.vitamin_a_mcg = micros.vitamin_a_mcg *
        (ingredientData.gramsPerUnit ? grams : 1);
    }
    if (micros.vitamin_c_mg) {
      nutrients.vitamin_c_mg = micros.vitamin_c_mg *
        (ingredientData.gramsPerUnit ? grams : 1);
    }
    if (micros.vitamin_e_mg) {
      nutrients.vitamin_e_mg = micros.vitamin_e_mg *
        (ingredientData.gramsPerUnit ? grams : 1);
    }
    if (micros.calcium_mg) {
      nutrients.calcium_mg = micros.calcium_mg *
        (ingredientData.gramsPerUnit ? grams : 1);
    }
    if (micros.iron_mg) {
      nutrients.iron_mg = micros.iron_mg *
        (ingredientData.gramsPerUnit ? grams : 1);
    }
    if (micros.potassium_mg) {
      nutrients.potassium_mg = micros.potassium_mg *
        (ingredientData.gramsPerUnit ? grams : 1);
    }
  }
...
async function estimateNutrition(
  ...
  const micronutrients: MicronutrientData = {
    vitamin_a_mcg: totals.vitamin_a_mcg > 0
      ? Math.round(totals.vitamin_a_mcg / servings)
      : undefined,
    vitamin_c_mg: totals.vitamin_c_mg > 0
      ? Math.round(totals.vitamin_c_mg / servings)
      : undefined,
    vitamin_e_mg: totals.vitamin_e_mg > 0
      ? Math.round((totals.vitamin_e_mg / servings) * 10) / 10
      : undefined,
    calcium_mg: totals.calcium_mg > 0
      ? Math.round(totals.calcium_mg / servings)
      : undefined,
    iron_mg: totals.iron_mg > 0
      ? Math.round((totals.iron_mg / servings) * 10) / 10
      : undefined,
    potassium_mg: totals.potassium_mg > 0
      ? Math.round(totals.potassium_mg / servings)
      : undefined,
  };
  ...
  return {
    ...
    servings,
    insights:
      `Nutrition estimates based on ${matchedCount}/${ingredients.length} matched ingredients. Calorie source: ${calorieSource}.`,
  };
}
...
- All text labels, headers, and descriptions must be translated`;
  ...
  const userPrompt =
    `Format this nutrition analysis for the recipe "${recipeName}":
...
${
      nutrition.micronutrients.vitamin_a_mcg
        ? `- Vitamin A: ${nutrition.micronutrients.vitamin_a_mcg}µg`
        : ""
    }
${
      nutrition.micronutrients.vitamin_c_mg
        ? `- Vitamin C: ${nutrition.micronutrients.vitamin_c_mg}mg`
        : ""
    }
${
      nutrition.micronutrients.calcium_mg
        ? `- Calcium: ${nutrition.micronutrients.calcium_mg}mg`
        : ""
    }
${
      nutrition.micronutrients.iron_mg
        ? `- Iron: ${nutrition.micronutrients.iron_mg}mg`
        : ""
    }
${
      nutrition.micronutrients.potassium_mg
        ? `- Potassium: ${nutrition.micronutrients.potassium_mg}mg`
        : ""
    }
...
**Analysis Notes:** ${
      nutrition.insights || "Nutrition estimates based on ingredient database."
    }
...
Remember: Format this beautifully in the SAME LANGUAGE as the recipe name "${recipeName}". Use appropriate headers, emojis, and make it easy to read.`;
  ...
  try {
    ...
    return openaiData.choices[0].message.content ||
      "Error formatting nutrition data";
  } catch (error) {
...
const handler = async (req: Request) => {
  ...
  try {
    ...
    console.log(
      `[TheLoop Nutrition] Analysis complete: ${result.perServingNutrition.calories} cal/serving, confidence: ${result.confidenceLevel}`,
    );
