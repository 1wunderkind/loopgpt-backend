...
const handler = async (req: Request) => {
  try {
    ...
    const comparison = {
      ...
      winner: {
        higher_protein: foodA.nutrition.protein_g > foodB.nutrition.protein_g
          ? food_a
          : food_b,
        lower_calories: foodA.nutrition.calories < foodB.nutrition.calories
          ? food_a
          : food_b,
        higher_fiber:
          (foodA.nutrition.fiber_g || 0) > (foodB.nutrition.fiber_g || 0)
            ? food_a
            : food_b,
      },
    };
