...
const handler = async (req: Request) => {
  try {
    ...
    if (!field) {
      return new Response(
        JSON.stringify({
          success: false,
          error:
            `Unknown nutrient: ${nutrient_name}. Try: protein, carbs, fat, fiber, calories`,
    ...
    const sorted = allFoods
      .filter((food: any) =>
        food.nutrition[nutritionField] && food.nutrition[nutritionField] > 0
      )
      .sort((a: any, b: any) =>
        (b.nutrition[nutritionField] || 0) - (a.nutrition[nutritionField] || 0)
      )
      .slice(0, 20); // Top 20
