...
import {
  convertToGrams,
  lookupFoodForTracker,
} from "../../lib/tracker_food_integration.ts";
  ...
  try {
    ...
    if (!chatgpt_user_id || !food_name || !quantity || !quantity_unit) {
      return new Response(
        JSON.stringify({
          error:
            "Missing required fields: chatgpt_user_id, food_name, quantity, quantity_unit",
        }),
        {
          status: 400,
    ...
    if (foodLookup.source !== "not_found") {
      ...
      protein_g = parseFloat(
        (foodLookup.protein_per_100g * multiplier).toFixed(1),
      );
...
async function updateDailySummary(
  ...
  const totals = logs.reduce((acc: any, log: any) => ({
    ...
    breakfast_calories: acc.breakfast_calories +
      (log.meal_type === "breakfast" ? log.calories : 0),
    lunch_calories: acc.lunch_calories +
      (log.meal_type === "lunch" ? log.calories : 0),
    dinner_calories: acc.dinner_calories +
      (log.meal_type === "dinner" ? log.calories : 0),
    snack_calories: acc.snack_calories +
      (log.meal_type === "snack" ? log.calories : 0),
  }), {
    total_calories: 0,
    total_protein_g: 0,
    total_carbs_g: 0,
...
async function updateUserStats(client: any, userId: string, logDate: string) {
  ...
  const daysDiff = Math.floor(
    (currentLogDate.getTime() - lastLogDate.getTime()) / (1000 * 60 * 60 * 24),
  );
