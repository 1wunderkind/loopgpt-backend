...
function generateInsights(
  ...
  if (caloriesProgress < 50) {
    ...
    insights.push(
      "You've exceeded your calorie goal. Consider lighter options.",
    );
  ...
  if (proteinProgress < 50) {
    insights.push(
      "Low protein intake. Add lean meats, eggs, or protein shakes.",
    );
  } else if (proteinProgress >= 100) {
    insights.push(
      "Great protein intake! This supports muscle growth and recovery.",
    );
  }
  ...
  if (stats && stats.current_streak_days >= 7) {
    insights.push(
      `Amazing! You're on a ${stats.current_streak_days}-day logging streak! 🔥`,
    );
