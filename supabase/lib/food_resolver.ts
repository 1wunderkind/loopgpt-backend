...
class FoodResolver {
  ...
  public async load(): Promise<void> {
    ...
    console.log(
      `✅ Food database loaded: ${this.foods.length} foods, ${
   ...
   */
  public async findFuzzy(
    query: string,
    limit: number = 10,
  ): Promise<Array<{ food: Food; score: number }>> {
    await this.load();
    ...
    console.log(
      `🔍 Fuzzy search "${query}" → ${results.length} results (${searchTime}ms)`,
    );
 ...
 */
export function initFoodResolver(
  cdnBaseUrl: string,
  version: string = "v1",
): FoodResolver {
