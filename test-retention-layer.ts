...
async function testRetentionLayer() {
  ...
  try {
    const response2 = await fetch(
      `${SUPABASE_URL}/functions/v1/mcp-tools/tools/retention.dailySuggestion`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  try {
    const response4 = await fetch(
      `${SUPABASE_URL}/functions/v1/mcp-tools/tools/retention.weeklyRefresh`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  try {
    ...
    console.log(
      `Last plan date: ${result5.profile?.lastPlanDate || "Not set"}\n`,
    );
  } catch (error) {
    console.error(`❌ Error: ${error.message}\n`);
  }
