...
async function testSentimentLayer() {
  ...
  try {
    const response1 = await fetch(
      `${SUPABASE_URL}/functions/v1/mcp-tools/tools/feedback.sentiment`,
      {
        method: "POST",
        headers: {
  ...
  try {
    const response5 = await fetch(
      `${SUPABASE_URL}/functions/v1/mcp-tools/tools/feedback.getFavorites`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    console.log(
      `Favorites: ${JSON.stringify(result5.favorites || [], null, 2)}\n`,
    );
  } catch (error) {
    console.error(`❌ Error: ${error.message}\n`);
  }
  ...
  try {
    const response6 = await fetch(
      `${SUPABASE_URL}/functions/v1/mcp-tools/tools/feedback.getStats`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
