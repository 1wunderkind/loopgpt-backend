...
Deno.test("Exact food lookup works", async () => {
  const { initFoodResolver, getFoodResolver } = await import(
    "../supabase/lib/food_resolver.ts"
  );
...
Deno.test("Fuzzy food search works", async () => {
  ...
  const hasChicken = results.some((r) =>
    r.food.name.toLowerCase().includes("chicken")
  );
...
Deno.test("Tracker food lookup with fallback works", async () => {
  const { lookupFoodForTracker } = await import(
    "../supabase/lib/tracker_food_integration.ts"
  );
  ...
  assertEquals(
    result.source === "resolver" || result.source === "database",
    true,
  );
});
...
Deno.test("Unit conversion works correctly", async () => {
  const { convertToGrams } = await import(
    "../supabase/lib/tracker_food_integration.ts"
  );
