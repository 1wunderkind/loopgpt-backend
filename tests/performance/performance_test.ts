...
Deno.test("performance: order routing responds under 2000ms", async () => {
  ...
  assert(
    duration < 2000,
    `Response time ${duration}ms exceeds 2000ms threshold`,
  );
});
...
Deno.test("performance: handles 100 requests per minute", async () => {
  ...
  console.log(
    `100 requests completed in ${duration}ms (${
      (100 / (duration / 1000)).toFixed(2)
    } req/s)`,
  );
...
Deno.test("performance: maintains response time under load", async () => {
  ...
  assert(
    maxDuration < avgDuration * 3,
    `Max response time ${maxDuration}ms too high`,
  );
});
...
Deno.test("performance: memory usage stays stable under load", async () => {
  ...
  console.log(
    `Memory increase after 100 requests: ${memoryIncreaseMB.toFixed(2)}MB`,
  );
...
Deno.test("performance: connection reuse works", async () => {
  ...
  console.log(
    `First: ${firstDuration}ms, Avg later: ${avgLaterDuration.toFixed(2)}ms`,
  );
