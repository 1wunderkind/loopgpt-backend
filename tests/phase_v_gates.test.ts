...
Deno.test("Gate: Nutrition Input Validation - Throws on missing recipes", () => {
  try {
    ...
    if (
      !(e instanceof Error) || !e.message.includes("recipes array is required")
    ) {
      throw e;
    }
  }
...
Deno.test("Gate: MealPlan Input Validation - Throws on missing goals", () => {
  try {
    ...
    if (
      !(e instanceof Error) || !e.message.includes("goals object is required")
    ) {
      throw e;
    }
  }
