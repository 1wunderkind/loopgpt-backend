...
} from "../supabase/functions/mcp-tools/nutrition.ts";
import {
  generateMealPlan,
  validateMealPlanInput,
} from "../supabase/functions/mcp-tools/mealplan.ts";
import { Logger } from "../supabase/functions/_shared/monitoring/Logger.ts";
import {
  ErrorHandler,
  ValidationError,
} from "../supabase/functions/_shared/errors/ErrorHandler.ts";
...
Deno.test("Regression: Runtime Contract - Nutrition Input", () => {
  ...
  try {
    ...
    assertEquals(
      (e as Error).message,
      "recipes array is required and must not be empty",
    );
  }
});
