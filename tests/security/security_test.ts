...
import { callEdgeFunction, parseResponse } from "../integration/helpers.ts";
...
Deno.test("security: checks token expiration", async () => {
  ...
  const expiredToken =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwiZXhwIjoxNTE2MjM5MDIyfQ.xxx";
...
Deno.test("security: rate limits authentication attempts", async () => {
  ...
  console.log(
    `Rate limiting ${rateLimited ? "working" : "not implemented yet"}`,
  );
});
