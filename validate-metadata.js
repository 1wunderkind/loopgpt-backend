...
test("theloopgptMetadata.ts has app name", () => {
  if (
    ...
    !metadataContent.includes("TheLoopGPT")
  ) {
...
const toolMatches = toolsContent.match(
  /export const TOOL_\w+: ToolDescription/g,
);
...
test(`routingHints.ts has ${triggerHintCount} trigger hints`, () => {
  if (triggerHintCount < 15) {
    throw new Error(
      `Expected at least 15 trigger hints, found ${triggerHintCount}`,
    );
  }
});
...
test("index.ts exports metadata", () => {
  if (
    ...
    !indexContent.includes("THELOOPGPT_METADATA")
  ) {
